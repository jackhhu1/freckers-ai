...
class LightweightBoard:
    ...
    def evaluate(self, color: PlayerColor) -> float:
        ...
        score = (opp_dist - self_dist) * 5.0 + (jump_self - jump_opp) * 0.1
        return score
...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        # Determine goal row for this agent
        ...
        for act in legal:
            ...
            if clone.is_win(self.color):
                return act
        # Detect near-win to avoid GrowAction
        near_win = False
        advance_dirs = {d for d in VALID_DIRECTIONS[self.color]
                        if (d.value.r == 1 and self.color == PlayerColor.RED) or
                           (d.value.r == -1 and self.color == PlayerColor.BLUE)}
        for frog in self.board.frogs[self.color]:
            if (self.color == PlayerColor.RED and frog.r == goal_row - 1) or \
               (self.color == PlayerColor.BLUE and frog.r == goal_row + 1):
                for d in advance_dirs:
                    try:
                        dest = frog + d
                        if self.board.is_lilypad(dest):
                            near_win = True
                            break
                    except ValueError:
                        continue
            if near_win:
                break
        if near_win:
            legal = [m for m in legal if not isinstance(m, GrowAction)]
        # Dynamic depth based on frogs at goal row
        frogs_finished = sum(1 for f in self.board.frogs[self.color] if f.r == goal_row)
        if frogs_finished >= 5:
            depth_limit = 7
        elif frogs_finished >= 3:
            depth_limit = 5
        else:
        ...
        for act in legal:
            clone = self.board.clone()
            clone.apply_action(self.color, act)
            value = self.minimax(clone, depth_limit - 1, alpha, beta, False, start)
            if value > best_value:
                best_value = value
                best_action = act
    ...
    def minimax(self, board: LightweightBoard, depth: int, alpha: float, beta: float,
        ...
        if maximizing:
            ...
            for action in actions:
                ...
                child_val = self.minimax(clone, depth - 1, alpha, beta, False, start_time)
            ...
            for action in actions:
                clone = board.clone()
                clone.apply_action(current_player, action)
                child_val = self.minimax(clone, depth - 1, alpha, beta, True, start_time)
