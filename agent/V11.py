    ...
    def action(self, **referee: dict) -> Action:
        ...
        for act in legal:
            ...
            if clone.is_win(self.color):
                return act
        # Detect near-win: frog one step from goal with available move
        goal_row = BOARD_SIZE-1 if self.color == PlayerColor.RED else 0
        near_win = False
        # directions that advance towards goal
        advance_dirs = {d for d in VALID_DIRECTIONS[self.color] if (d.value.r == 1 if self.color == PlayerColor.RED else d.value.r == -1)}
        for frog in self.board.frogs[self.color]:
            # check if frog is one step away
            if (self.color == PlayerColor.RED and frog.r == goal_row-1) or (self.color == PlayerColor.BLUE and frog.r == goal_row+1):
            ...
            if near_win:
                break
        # If near-win, remove GrowAction to prioritize finishing
        if near_win:
            legal = [m for m in legal if not isinstance(m, GrowAction)]
        # Otherwise, normal minimax selection
        start = time.time()
