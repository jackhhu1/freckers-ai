...
class LightweightBoard:
    ...
    def _apply_move(self, color: PlayerColor, action: MoveAction):
        ...
        for direction in action.directions:
            # Determine if it's a jump by checking for a frog in the mid-point
            mid = pos + direction
    ...
    def evaluate(self, color: PlayerColor) -> int:
        ...
        score = 0
        for frog in self.frogs[color]:
            score += frog.r if color == PlayerColor.RED else (7 - frog.r)
        for frog in self.frogs[enemy]:
            score -= frog.r if enemy == PlayerColor.RED else (7 - frog.r)
        return score
